...
# =============================================================================
# ADR-018 RBAC Shared First — MONITORING-API (P1)
# App-level enforcement: 1 Depends() global para 18 endpoints.
# =============================================================================
_MON_RBAC_GUARD = None
_MON_SHARED_OK = False
try:
    ...
    try:
        _MON_RBAC_GUARD = default_guard_from_env(audience_env="OTK_AUDIENCE")
        _MON_SHARED_OK = True
...
def _mon_get_rbac_guard():
    return _MON_RBAC_GUARD
...
_MON_ROLE_STRIP_PREFIXES = ("OTK_", "ONTK_", "ONTRACKCHAIN_", "B2B_")


def _mon_normalize_role(role_raw: str | None) -> str:
    if not role_raw:
        return ""
    r = str(role_raw).strip().upper()
    for pfx in _MON_ROLE_STRIP_PREFIXES:
        if r.startswith(pfx):
            r = r[len(pfx):]
    return _MON_ROLE_ALIASES.get(r, r)
...
async def _require_role_with_audit(
    ...
    *,
    ...
    guard = _mon_get_rbac_guard()
    normalized_candidate: str = ""
    source: str = ""
    valid_jwt_roles: set[str] = set()
    if guard is not None and _MON_SHARED_OK and authorization:
        try:
            ...
            valid_jwt_roles = {_mon_normalize_role(r) for r in roles_claim if r}
        except Exception:  # noqa: BLE001
            valid_jwt_roles = set()
    if valid_jwt_roles:
    ...
    if not normalized_candidate:
        normalized_candidate = _mon_normalize_role(x_role)
        source = "x-role"
    allowed_normalized = {_mon_normalize_role(r) for r in allowed_roles}
    if normalized_candidate not in allowed_normalized:
        ...
        logger.warning("RBAC_DENIAL monitoring-api ep=%s m=%s req=%s rcv=%r src=%s", endpoint, method, sorted(allowed_normalized), normalized_candidate, source)
        raise HTTPException(status_code=403, detail=detail_msg)
    return normalized_candidate


ANALYST_READ = {"ADMIN", "AUDITOR", "ANALYST", "VIEWER", "INVESTIGATOR", "COMPLIANCE"}
WRITE_OPS = {"ADMIN", "ANALYST", "INVESTIGATOR", "COMPLIANCE"}

_RBAC_ROUTE_POLICIES: dict[tuple[str, str], set[str]] = {
    ("/monitoring/v1/webhook/alertmanager", "POST"): {"ADMIN", "AUDITOR"},
}
_RBAC_PREFIX_POLICIES: list[tuple[str, set[str], set[str] | None]] = [
    ("/monitoring/v1/watchlists/", ANALYST_READ, {"GET"}),
    ("/monitoring/v1/watchlists/", WRITE_OPS, {"POST", "PUT", "DELETE", "PATCH"}),
    ("/monitoring/v1/alerts/", ANALYST_READ, {"GET"}),
    ("/monitoring/v1/alerts/", WRITE_OPS, {"POST", "PUT", "DELETE", "PATCH"}),
    ("/monitoring/v1/quotes/", ANALYST_READ, {"GET", "POST"}),
    ("/monitoring/v1/aml/", ANALYST_READ, {"GET"}),
    ("/monitoring/v1/aml/", WRITE_OPS, {"POST", "PUT", "PATCH", "DELETE"}),
    ("/monitoring/v1/sanctions/", ANALYST_READ, {"GET", "POST"}),
    ("/monitoring/v1/internal/", {"ADMIN"}, None),
]
...
async def _app_rbac_enforcer(
    ...
    if key in _RBAC_ROUTE_POLICIES:
        await _require_role_with_audit(
            ...
            detail=f"mon_{scope_path.strip('/').replace('/', '_')}_forbidden",
        )
        return
    for prefix, allowed, methods_filter in _RBAC_PREFIX_POLICIES:
        if scope_path.startswith(prefix) and (methods_filter is None or method in methods_filter):
            await _require_role_with_audit(
                ...
                endpoint=scope_path, method=method,
                detail=f"mon_{prefix.strip('/').replace('/', '_')}_forbidden",
            )
            return
...
try:
    from ontrackchain_shared.logging_util import (
        RequestIdLogMiddleware as _MonitoringRequestIdLogMiddleware,
        setup_structured_logging,
    )
    _MONITORING_STRUCTURED_LOG_OK = True
    ...
    _MonitoringRequestIdLogMiddleware = None  # type: ignore[assignment, misc]
    _MONITORING_STRUCTURED_LOG_OK = False

if setup_structured_logging is not None:
    import os as _monitoring_os
    setup_structured_logging(
        "monitoring-api",
        level=_monitoring_os.environ.get("LOG_LEVEL", "INFO"),
    )
    del _monitoring_os
...
app = FastAPI(
    title="OnTrackChain Monitoring API",
    dependencies=[Depends(_app_rbac_enforcer)],
)


# Sprint S28+53 P3: Request ID Middleware. Injeta request_id nos logs estruturados.
if _MONITORING_STRUCTURED_LOG_OK and _MonitoringRequestIdLogMiddleware is not None:
    app.add_middleware(_MonitoringRequestIdLogMiddleware)
...
async def trigger_operational_alert(
    ...
    _require_role_with_audit(
        ...
        resource_type="operational_alerts",
        resource_id=None,
        endpoint="/api/v1/monitoring/test/trigger-operational-alert",
        method="POST",
    )
...
async def healthz_liveness_probe():
    ...
    body = {
        ...
        "service_id": f"ontrackchain/monitoring-api",
    }
    return Response(
        status_code=200,
...
fastapi_info{service="monitoring-api",version="3.1.0-m5"} 1.0
