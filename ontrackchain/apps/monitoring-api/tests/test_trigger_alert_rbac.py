...
class MonitoringTriggerAlertRbacTests(unittest.TestCase):
    ...
    def test_require_test_trigger_role_allows_tester(self) -> None:
        state, pool = self._build_state()

        normalized_role = main._require_test_trigger_role_with_audit(
            pool,
            organization_id="org-1",
            user_id="11111111-1111-1111-1111-111111111111",
            external_user_id=None,
            request_id="req-monitoring-tester-1",
            x_role="TESTER",
    ...
    def test_require_test_trigger_role_rejects_analyst_and_records_denial(self) -> None:
        state, pool = self._build_state()

        with self.assertRaises(main.HTTPException) as ctx:
            main._require_test_trigger_role_with_audit(
                pool,
                organization_id="org-1",
                user_id="11111111-1111-1111-1111-111111111111",
                external_user_id=None,
                request_id="req-monitoring-analyst-1",
                x_role="ANALYST",
                resource_id="watchlist-1",
                endpoint="/api/v1/monitoring/test/trigger-alert",
                method="POST",
        ...
        self.assertEqual(ctx.exception.detail, "monitoring_test_trigger_role_required")
        ...
        self.assertEqual(log_entry["resource_type"], "monitoring_test_alert")
        self.assertEqual(log_entry["resource_id"], "watchlist-1")
        self.assertEqual(log_entry["metadata"]["effective_role"], "ANALYST")
        self.assertEqual(log_entry["metadata"]["detail"], "monitoring_test_trigger_role_required")
