...
Serviço FastAPI **OnTrackChain Report API** (v3.1.0-m5) — parte da plataforma Ontrackchain Regtech.

## Portas e base path

- Porta padrão: `8004` (Docker EXPOSE 8004)
- Base path (root_path): `/api/v1/reports`
...
| Feature | Detalhe |
|---|---|
| Relatórios regulatórios PDF/XML/CSV | Relatórios regulatórios PDF/XML/CSV |
| ROS-COAF | ROS-COAF (Banco Central) |
| Strong Auth | Strong Auth (WebAuthn TOTP — ADR-004) para exportação |
| Formal Dossier com assinatura eletrônica | Formal Dossier com assinatura eletrônica |
| Regulatory Auto Filing | Regulatory Auto Filing (SPED, SISCOAF, BACEN-JUD) |
...
python3 -m pip install -e "ontrackchain/apps/report-api[dev,report,strongauth]"
```

Rodar API via uvicorn:

```bash
cd ontrackchain/apps/report-api
uvicorn report_api.main:app --host 0.0.0.0 --port 8004 --reload
```
...
```bash
docker build -f ontrackchain/apps/report-api/Dockerfile -t ontrackchain-report-api .
```
...
- **Runtime**: `APP_ENV=(dev\|qa\|staging\|prod)`, `REPORT_API_PORT`
