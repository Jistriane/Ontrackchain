# ADR-013: Digest canônico do Export no Showcase E2E
...
- o hash canônico emitido imediatamente pela resposta HTTP do export
...
Adotar como baseline para o showcase E2E a `Opcao B`: tratar a resposta do export como fonte canônica imediata do digest e usar o DOM apenas como confirmacao de convergencia visual.
...
2. o fluxo de `signoff-request` e `finalize` deve usar esse digest canônico, nao o primeiro texto disponivel no DOM;
3. o teste deve aguardar a UI renderizar o mesmo hash antes de seguir, preservando validação visual sem depender de estado transitorio;
...
- alinha o helper E2E ao contrato canônico do proprio endpoint de export
...
- foi evitada mudanca estrutural no frontend de producao, porque o problema residia no ponto de observação do teste
- foi aceita dependencia controlada do header HTTP, pois ele ja representa o artefato canônico emitido pelo backend

## Criterios de Aceitacao

- o helper E2E captura o digest canônico da resposta de export
