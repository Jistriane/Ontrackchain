# ADR-007 — validação por Modo de autenticação

## Contexto

O projeto passou a operar com dois modos de autenticação explicitamente distintos:

- `oidc`, que representa o caminho canônico para ambientes serios
...
- validação canônica de `OIDC` + `RBAC` + identidade federada
- validação legada/local de `2FA` e `dev_jwt`

Isso dificultava interpretar falhas e misturava regras de segurança com semanticas diferentes.

## Decisao

Adotar uma estrategia de validação em dois trilhos no CI/CD:

- um gate `OIDC` explicito e prioritario para os fluxos canônicos de ambiente serio
...
- o gate `OIDC` continua sendo parte obrigatoria da validação para ambiente serio
- o job `dev auth` existe como regressao controlada do scaffold local

## Motivacao

- separar claramente autenticação canônica de autenticação local de compatibilidade
...
- tornar as evidências de release mais legiveis para auth, compliance e operação
...
- Desvantagem:
  - mistura semanticas de segurança diferentes
...
- Desvantagem:
  ...
  - exige documentação clara para evitar uso incorreto do job local como criterio de release
...
- o time passa a tratar `OIDC` como gate principal de autenticação em CI
...
- mais tempo de pipeline em troca de evidências mais claras por modo de autenticação
