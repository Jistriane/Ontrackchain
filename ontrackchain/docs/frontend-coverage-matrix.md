...
| Monitoramento | pronta | hub `/monitoring` agora orquestra watchlists e alertas de teste via `use-monitoring-watchlist-alerts.ts` + `watchlist-alerts-panel.tsx`, loaders centralizados em `app/lib/monitoring-api.ts`, hook dedicado para `platform alerts` (`use-monitoring-platform-alerts.ts`), hook dedicado para `worker + operational alerts + DLQ` (`use-monitoring-operations.ts`), paineis apresentacionais extraidos para triagem, operacoes e remediacao, export auditado e deep-links compartilhados para case/audit/evidence, com UX preventiva degradando leituras administrativas para `ADMIN/AUDITOR` e ocultando mutações/exportações fora de `ADMIN`, além de cobertura E2E focal para `trigger-alert`, read-only de `AUDITOR` e bloqueio precoce para `ANALYST` | incident response continua acoplado a mesma rota `/monitoring` e ainda nao foi promovido a modulo/rota propria |
