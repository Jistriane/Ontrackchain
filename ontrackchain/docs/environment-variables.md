...
- execute `python3 scripts/check_staging_env_ownership_coverage.py --env-file .env.staging.example --ownership-file docs/staging-env-ownership.md` para garantir que nenhum placeholder novo entrou no baseline sem owner, apoio e evidência
...
| `AUTH_MODE` | `dev` | modo global de autenticação do scaffold |
...
| `INVESTIGATION_RPC_ENABLED` | `false` | habilita integração RPC explicita no worker de investigation |
...
| `COMPLIANCE_COMPLETED_WITHOUT_REPORT_WARN_THRESHOLD` | `3` | limiar para casos concluidos sem relatório persistido |
...
| `REPORT_DOWNLOADS_LAST_24H_WARN_THRESHOLD` | `10` | limiar de alerta para volume alto de downloads de relatório |
...
| `REPORT_PERSISTED_WITHOUT_DOWNLOAD_WARN_THRESHOLD` | `3` | limiar para relatórios persistidos ainda sem download auditado |
...
| `MFA_TOTP_WINDOW` | `1` | tolerancia de janelas adjacentes para validação do TOTP |
...
- o preset `generic` continua util como fallback tecnico, mas nao e mais a referencia principal do próximo incremento
...
5. `OIDC_AUDIENCE=ontrackchain-api` para a validação das APIs
...
| `INTERNAL_AUTH_BASE_URL` | `http://auth-service:9000` | override opcional para validação direta de token em proxies server-side sensiveis |
