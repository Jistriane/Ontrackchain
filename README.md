...
- release atual: `v5.12.0 Sprint 27 Ajustes Governança Final: Relatório Final Consolidado S1→S27 9 Seções + M5-removal Sign-off Preenchido 70% Pronto Jurídico Assinar (Baseline v1.7 99% mantido NENHUM código novo)` (CHANGELOG v5.12.0 nova entrada, Relatório Final anexo único tudo-em-um ANPD BACEN, M5 preenchido estrutural campos SHA 1a7590a ahead 24→25 data validade 2026-08-10→2026-08-12 48h ahead origin/main 23→25)
...
| **`S28-A1-GAP-PYTEST-44-PASS`** Sprint 28+0: pytest 44/44 contrato em sandbox isolado /tmp/otk-venv (python 3.14) (12 Q3-08/Q3-09 qa-gateway TruffleHog + Q3-09 ENFORCE_ALL; 12 T2-10/T2-11 Billing; 16 T2-12 Integrated 8 rotas enforcement) + 8 BUGs Críticos Descobertos e Corrigidos (maior: FastAPI Depends(lambda r: ...) 422 Field required substituído por 8 async named function wrappers; NameError qa-gateway cli.py 3x @app.command → @cli.command; ImportError billing_stripe alias compatibilidade str→UUID; email-validator missing; TruffleHog dry-run exit 0 sempre agora; T2-11 assertion remaining 0; 14 testes T2-12 bodies Pydantic obrigatórios) | `done` | 4 bugs de código + 4 bugs de teste corrigidos. GAP-A1 100% fechado. Sem lambda anônimo em Depends() mais (regra projetual institucionalizada). M5 commits locais ahead origin/main: 25→26. |
| **`S28-A2-GAP-TRUFFLEHOG-DRY-RUN`** Sprint 28+0: qa-gateway scan-secrets-trufflehog --dry-run local executado, exit 0, TS-W001 (sem binário no host → esperado). STRICT ignorado dry-run. | `done` | Q3-08 operacional; instalar binário no runner CI é único item pendente P0-03 antes de push remoto. |
...
| **`S28-DASH-HANDOFF-EXECUTIVO`** Sprint 28+0: Dashboard Handoff Executivo HANDOFF-DASHBOARD-P0-01-02-03-2026-08-10.md NOVO: Contagem regressiva validade M5 48h (expira 2026-08-12 23:59 BRT), Resumo GAP P0-01 (OIDC 68% 8-21d) / P0-02 (AML/BACEN 59% 7-14d) / P0-03 (Infra/M5 96% 1-3d), Calendário implantação 3 cenários 16-24-38 dias úteis, Métricas confiança, Checklist 4-olhos M5, Notas Arquiteto Bug Lambda Depends descoberto. | `done` | Pronto para acompanhamento diário diretoria até push remoto. |

### Bloqueadores para o salto regulatório

- `M5 Push Remoto (🔴 BLOQUEIO ABSOLUTO)`: sincronizar 26 commits locais da branch `main` com GitHub origin/main — proibido qualquer `git push` remoto até autorização explícita formal + método definido (PAT SSO, SSH deploy key ou Render GitHub App) + sign-off M5-removal completo 6 pessoas + Procedimento 14 passos executados (arquivo docs/governance-sign-offs/M5-removal-2026-08-10-HEAD-24-COMMITS.md preenchido)
