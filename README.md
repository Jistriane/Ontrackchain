...
- release atual: `v5.10.0 Sprint 26 ADR-029 Pre-Merge 5 Gates FAIL-FAST + LGPD RIPD Art.15 Master + Template B2B Por Cliente + qa-gateway Q3-08 scan-secrets-trufflehog + Q3-09 run-pre-merge-gates + 12 pytest contrato (Baseline v1.5 97%→98%)` (ADR-029 7 seções + Mermaid flowchart LR, RIPD mestre 16 campos, RIPD template 17 campos específicos cliente, scan-secrets TS-E001/E002 3 issues, orquestrador Q5 SEMPRE roda, ahead origin/main 21→22)
...
- `M5 Push Remoto (🔴 BLOQUEIO ABSOLUTO)`: sincronizar 22 commits locais da branch `main` com GitHub origin/main — proibido qualquer `git push` remoto até autorização explícita formal + método definido (PAT SSO, SSH deploy key ou Render GitHub App)
