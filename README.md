...
- release atual: `v5.8.0 Sprint 24 Billing Enforcement Middleware Redis Fail-Closed 402 + qa-gateway Q3-06 + Handbook OIDC Keycloak v25 Helm + ADR-027 Governança 27 ADRs` (enforcement 3 capabilities T2-11, 15 pytest contrato, qa-gateway scan-billing-enforcement 4 warnings BE-001..004, handbook P0-01 14 itens checklist 4-eyes, baseline v1.3 95%→96%, ahead origin/main 19→20)
...
- `M5 Push Remoto (🔴 BLOQUEIO ABSOLUTO)`: sincronizar 20 commits locais da branch `main` com GitHub origin/main — proibido qualquer `git push` remoto até autorização explícita formal + método definido (PAT SSO, SSH deploy key ou Render GitHub App)
