...
6.  `g6 typecheck`               → mypy **STRICT 4 MÓDULOS (Shared + QA + Agents + auth_service)** S31/S32/S33/S37 P3: `ontrackchain_shared.*` + `qa_gateway.*` + `ontrackchain_agents.*` + `auth_service.*` = disallow_untyped_defs=true; 8 apps restantes = baseline. 4 camadas fallback (mypy/PYmod/hatch/hatch PYmod)
