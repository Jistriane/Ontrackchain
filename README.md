...
- release atual: `v5.9.0 Sprint 25 LGPD ROPD Art.37 7 Operações + Billing Enforcement Integrado 8 Rotas Reais + qa-gateway scan-lgpd-ropd Q3-07 + Template Sign-off M5 Governança Risco` (7 ROPDs + CSV consolidado, 4 routers novos SRP (ai, b2b, users, graph), enforcement estimate/start, 16 pytest T2-12, scan-lgpd LR-001..005, template sign-off M5, baseline 96→97%, ahead origin/main 20→21)
...
- `M5 Push Remoto (🔴 BLOQUEIO ABSOLUTO)`: sincronizar 21 commits locais da branch `main` com GitHub origin/main — proibido qualquer `git push` remoto até autorização explícita formal + método definido (PAT SSO, SSH deploy key ou Render GitHub App)
