...
- release atual: `v5.7.0 Sprint 23 Governança Arquitetura 26 ADRs + Billing Capabilities Matrix + qa-gateway scan-billing` (4 ADRs novos=22→26, Baseline Readiness v1.2 materialidade 90%→95%; billing capabilities OTK_PLAN_CAPABILITIES 3 tiers usage meters; qa-gateway scan-billing-capabilities BW-001..004 strict)
...
- `M5 Push Remoto (🔴 BLOQUEIO ABSOLUTO)`: sincronizar 19 commits locais da branch `main` com GitHub origin/main — proibido qualquer `git push` remoto até autorização explícita formal + método definido (PAT SSO, SSH deploy key ou Render GitHub App)
