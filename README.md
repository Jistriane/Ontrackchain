...
6.  `g6 typecheck`               → mypy **STRICT 5 MÓDULOS (Shared + QA + Agents + auth_service + compliance_api)** S31/S32/S33/S37/S38 P3: `ontrackchain_shared.*` + `qa_gateway.*` + `ontrackchain_agents.*` + `auth_service.*` + `compliance_api.*` = disallow_untyped_defs=true; 7 apps restantes = baseline. 4 camadas fallback (mypy/PYmod/hatch/hatch PYmod)
