...
6.  `g6 typecheck`               → mypy **🏁 STRICT GLOBAL DEFAULT ATIVADO S28+46! 12 MÓDULOS (3 shared: Shared/QA/Agents + 9 apps) + 9 flags estritas NO BLOCO DEFAULT `[tool.mypy]`. Qualquer NOVO módulo nasce strict automaticamente (fail-fast). 12 overrides individuais permanecem (defesa em profundidade = redundância boa).** S31/S32/S33/S37-S46 P4: roadmap mypy incremental 11/11 milestones FECHADO ✅! 4 camadas fallback (mypy/PYmod/hatch/hatch PYmod)
...
- 📋 **Guia Completo de Contribuição e Checklist Pré-Commit**: 5 hard constraints NÃO negociáveis (HC-1..HC-5 NOVO S28+61), ciclo sprint 5 passos, 8 gates FAIL-CLOSED, padrão de mensagem de commit e **Checklist 9 itens ANTES de git commit** em [CONTRIBUTING.md](./CONTRIBUTING.md) **[S28+55 + S28+61 P4 Hardening HC-5 Dotfiles + Trindade Docs]**
- 📜 `make changelog` → Histórico de 30 sprints (S28+29 → S28+61) em arquivo [CHANGELOG-SPRINTS.md](./CHANGELOG-SPRINTS.md): tabela resumo geral + detalhe por sprint ordem recente primeiro (S28+61 → S28+29) + legenda prioridades P0–P4. Target conveniência NÃO-gating com header + cat. **[S28+56 + S28+62 P4 Atualização 6 sprints recentes (S28+56→S28+61)]**
- 📒 `make contributing` → Guia completo de contribuição em [CONTRIBUTING.md](./CONTRIBUTING.md): 5 hard constraints NÃO negociáveis (HC-1..HC-5 NOVO S28+61) + ciclo sprint 5 passos (INV→DESIGN→IMPL→DOCS→VAL→COMMIT) + tabela completa 8 Gates FAIL-CLOSED + estrutura monorepo resumida + padrão mensagem de commit 6 blocos + **Checklist Rápido 9 itens ANTES de `git commit`** (3 NOVOS: Plugin EditorConfig ativo + Windows autocrlf + Trindade Docs opcional). Header bonito + cat arquivo simétrico ao changelog S28+56. **[S28+57 + S28+61 P4 HC-5 Dotfiles + 3 itens checklist]**
...
- 📁 **`.gitattributes` (SSOT Line Endings Cross-OS LF Force)** → Arquivo [.gitattributes](./.gitattributes) raiz: 3 blocos. BLOCO1 `* text=auto eol=lf` força LF em TODO arquivo texto (elimina CRLF Windows↔Linux diffs spurious). BLOCO2 CRÍTICOS sobrescrita explícita eol=lf para `*.sh/*.bash/Makefile/*.py/*.toml/*.yml/*.yaml/*.md/*.json` (previne G3 bash -n syntax error por `\r` + make missing separator por `\r` no fim receitas). BLOCO3 20 binários EXPLÍCITOS marcados `binary` (diff/EOL/merge desativados: 6 imagens + PDF + 2 bytecode Python + 6 arquivos compactados + 5 fontes). Lote temático com S28+58 dotfiles governança. **[S28+59]**
...
- 🩺 **DependsOn Healthy (Sprint S28+52 P3)**: 13 relações de dependência `service_started` → `condition: service_healthy`: CAT A 8 relações fastapi (investigation/redis, investigation-worker/investigation-api, compliance-worker/compliance-api, monitoring/redis, ai-service/redis, ai-worker/redis, case-management/ai-service) + CAT B prometheus depende 4 apps FastAPI + alertmanager/grafana estilo LIST → DICT. Resultado: 15× service_healthy / 10× service_completed_successfully / 2× service_started (prometheus→alertmanager, grafana→prometheus = 3rd-party sem HC definido). **Elimina 99% dos race conditions de startup no e2e-light script**.
- 🛡️ **Fail-safe 3rd-party**: serviços 3rd-party sem HEALTHCHECK Dockerfile ou compose YAML (`alertmanager`, `grafana`, `keycloak`, `traefik`) permanecem `service_started` ou sem depends_on para não bloquear compose indefinidamente por falta de HC.
