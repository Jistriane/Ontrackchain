...
- release atual: `v5.11.0 Sprint 27 Última Sprint Ciclo Implementação Código: CHANGELOG S1→S26 ADR-023 Cumprido + SIGNOFF-ADRS-ALL-29 Jurídico Consolidado + Workflow Pre-Merge ADR-029 Pronto (on:[]) (Baseline v1.6 98%→99%)` (CHANGELOG Keep a Changelog 1.1.0 11 releases hierárquicos, Signoff 29 ADRs Tabela + Assinatura 4-Olhos + Engenheiro, Workflow pre-merge 6 steps checkout→Python3.11→qa-gateway→TruffleHog→run-pre-merge-gates→artifact 180d LGPD, ahead origin/main 22→23)
...
- `M5 Push Remoto (🔴 BLOQUEIO ABSOLUTO)`: sincronizar 23 commits locais da branch `main` com GitHub origin/main — proibido qualquer `git push` remoto até autorização explícita formal + método definido (PAT SSO, SSH deploy key ou Render GitHub App)
